...
<div id="page-wrapper">
        ...
        <div class="col-lg-5">
            <div class="table-responsive">
                <table class="table table-bordered table-responsive">
                    ...
                    <tr><td><input class="categoryCheckBox" type="checkbox" value="{{{ $potentialparent->id }}}" /></td><td><strong>{{{ $potentialparent->name }}}</strong></td><td>{{{ $vaInCategories[$potentialparent->id] }}}</td>
                    @foreach($children as $child)
                    @if($child->parentid == $potentialparent->id)
                    <tr><td><input class="categoryCheckBox" type="checkbox" value="{{{ $child->id }}}" /></td><td><i class="fa fa-arrow-right fa-fw"></i> {{{ $child->name }}}</td><td>{{{ $vaInCategories[$child->id] }}}</td>
                    @endif
                    @endforeach
                    @endforeach
                </table>
        ...
        <div class="col-lg-5">
            <div class="table-responsive">
                <table class="table table-bordered table-responsive">
                    ...
                    <tr><td><input type="checkbox" class="auditorCheckBox" value="{{{ $auditor->cid }}}" /></td><td>{{{ $auditor->name }}}</td><td>{{{ $assignmentsPerAuditor[$auditor->cid] }}}</td></tr>
                    @endforeach
                </table>
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-lg-offset-5">
            <button class="btn btn-success">Submit Assignments</button>
        </div>
    </div>
</div>
