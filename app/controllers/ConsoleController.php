...
//        return View::make('console.login');
    ...
    public function post_ajaxsearch() {
        ...
        if (count($vas) > 0) {
            ...
            $send = '';
            foreach ($vas as $va) {
                $send .= "<tr><td>$va->vaname</td><td>$va->url</td><td>$va->cid</td><td>$va->name</td><td>$va->created_at</td><td><a class=\"searchEditVABtn\" href=\"" . URL::route('console') .  "/va/$va->cid\"><i class=\"fa fa-edit fa-fw\"</a></td></tr>";
            }
        }
        else {
            $send = '0';
